...
In questo paragrafo si esamineranno la relazione fra la Discrete-Time Fourier Transform (DTFT) e la N-point DFT.
...
\\
\\
Data la N-point DFT $X_k$ di una sequenza $x_n$ di lunghezza n è possibile determinare la DTFT $X(e^{j\omega _k})$ esequendo l'operazione di intepolazione sui valori di  $X_k$.

--> \emph{MITRA - PARAGRAFO 5.3.3}
...
Generalmente, per lo standar IEEE 734, operando in doppia precisione, la mantissa è lunga 52 bit per cui $u\approx 2.22 \times 10^{-16}$
...
Vediamo ora come calcolare i $\omega_N^k,\quad (k=0,1,...,N-1)$ con $N=2^n$ e $N_j=2^{-j}N, \quad (j=0,1,...,n)$. Usando le simmetrie degli esponenziali complessi $\omega_N^l \quad (l=N_3,...,N-1)$ si possono ottenere
\begin{align}
 \hat{\omega}_N^{N_2-k}=-i \overline{\hat{\omega}_N^{k}}, \quad &\text{simmetrie di $\pi/2$ tra i quadranti }\\ 
    \hat{\omega}_N^{N_2+k}=-i\hat{\omega}_N^{k},\quad&\text{ I e IV,I e II}\\
\hat{\omega}_N^{N_1-k}=\overline{\hat{\omega}_N^{k}},\quad& \text{simmetrie rispetto asse y}\\
\hat{\omega}_N^{N_1+k}=-\hat{\omega}_N^{k} \quad&
\end{align} 
per $k=1,..., N_3-1$  e da\[ \hat{\omega}_N^{N_3}=\hat{\omega}_8=fl\left(\sqrt{2}/2\right)(1-i),\quad \hat{\omega}_N^{N_2}=\hat{\omega}_4=-i,\quad \hat{\omega}_N^{N_1}=\hat{\omega}_2=-1
\]
 \subsection{Metodo diretto}
\subsection{Metodo ricorsivo}
\subsection{Repeated subvector scaling}
...
\chapter{FFT N-dimensionale e immagini}
\include{appendice}
