...
public class RtpPkt {
    ...
    private static final Logger LOGGER =
        Logger.getLogger(RtpPkt.class.getName());
    ...
    protected RtpPkt(long aTimeStamp, long syncSource, int seqNum, int plt, byte[] pl){
        ...
        test += setPayload(pl);
        if(test != 0) {
            LOGGER.warning("RtpPkt() failed, check with checkPkt()");
        }
        rawPktCurrent = true;
    ...
    protected RtpPkt(byte[] aRawPkt, int packetSize){
        ...
        if(aRawPkt == null) {
            LOGGER.warning("RtpPkt(byte[]) Packet null");
        }
        ...
        if(remOct >= 0) {
            ...
            if(version == 2) {
                ...
                LOGGER.warning("RtpPkt(byte[]) Packet is not version 2, giving up.");
            }
        } else {
            LOGGER.warning("RtpPkt(byte[]) Packet too small to be sliced");
        }
        rawPktCurrent = true;
        if(LOGGER.isLoggable(Level.FINEST)) {
            LOGGER.finest("<- RtpPkt(aRawPkt)");
    ...
    protected void printPkt() {
        ...
        LOGGER.finest("Timestamp:" + timeStamp + "(long output as int, may be 2s complement)");
        LOGGER.finest("SSRC:" + ssrc + "(long output as int, may be 2s complement)");
        for(int i=0;i<getCsrcCount();i++) {
            LOGGER.finest("CSRC:" + csrcArray[i] + "(long output as int, may be 2s complement)");
        }
        //TODO Extension
    ...
    protected int setSeqNumber(int number) {
        if(number <= 65536 && number >= 0) {
            ...
            LOGGER.warning("RtpPkt.setSeqNumber: invalid number");
            return -1;
        }
    }
    ...
    protected int setCsrcs(long[] contributors) {
        if(contributors.length <= 16) {
            ...
            LOGGER.warning("RtpPkt.setCsrcs: Cannot have more than 16 CSRCs");
            return -1;
        }
    }
    ...
    protected int setPayload(byte[] data) {
        ...
        if(data.length < (1500 - 12)) {
            ...
            LOGGER.warning("RtpPkt.setPayload: Cannot carry more than 1480 bytes for now.");
            return -1;
        }
    }
