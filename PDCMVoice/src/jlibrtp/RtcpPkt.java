...
public class RtcpPkt {
    ...
    private static final Logger LOGGER =
        Logger.getLogger(RtcpPkt.class.getName());
    ...
    protected boolean parseHeaders(int start) {
        ...
        if(LOGGER.isLoggable(Level.FINEST)) {
            LOGGER.finest(" <-> RtcpPkt.parseHeaders() version:"+version+" padding:"+padding+" itemCount:"+itemCount
        ...
        if(version == 2 && length < 65536) {
            ...
            LOGGER.warning("RtcpPkt.parseHeaders() failed header checks, check size and version");
            this.problem = -1;
            return false;
        }
    }
    ...
    protected void writeHeaders() {
        ...
        if(rawPkt.length % 4 != 0)
            LOGGER.warning("!!!! RtcpPkt.writeHeaders() rawPkt was not a multiple of 32 bits / 4 octets!");
    ...
    protected void encode() {
        LOGGER.warning("RtcpPkt.encode() should never be invoked!! " + this.packetType);
    }
