...
\caption[$m$ groups split: ALOHA scenario]{Results were obtained using MBT with optimal probability $p$ in a slotted ALOHA scenario. Splitting the batch into groups provides better results than applying a BRA to the original batch size when $y$-axis values below 1.}
\label{m-groups-MBT-ALOHA}
\end{center}
\end{figure}

Figure \ref{m-groups-MBT-ALOHA} shows that  $\rho\approx 1.26$ provides the best achievable performance. Furthermore we notice how the knowledge of the batch size is critical: $m$ can be set to the optimal value only knowing exactly the batch size $n$ but often we only have an estimate of $n$. If this estimate is smaller than $n$, performances smoothly degrade but still remain always better than the trivial application of the BRA. On the other hand overestimating $n$ can lead to performance loss when $\rho \approx 0.5$ or smaller.\\

\marginpar{RICHIEDO VERIFICA PER CMSA...\\ $\leftarrow$}
In CSMA scenario the performances depends on $\beta$ and on the ratio between the nodes packet size ($S_{n}$) and the supervisor feedback size ($S_{f}$). Figure \ref{m-groups-MBT-CSMA} was obtained considering $\displaystyle\frac{S_{n}}{S_{f}}\approx4$. We notice that for CSMA is better to have empty slots with high probability ($\rho$ small). CSMA is also much more robust to an overestimate of $n$ than \emph{slotted}-ALOHA.

\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.7]{matlab/BTs/m-groups-MBT-CSMA}
\caption[$m$ groups split: CSMA scenario]{IMMAGINE PROVVISORIA, $m$ groups split: CSMA scenario}
...
\item Start a new MBT at the next node at level $\lfloor\log_{2}\hat{n}\rfloor$.
...
\subsection{EBT combined with optimal MBT}

In this section we describe an improvement over EBT we developed.\\
EBT assumes the nodes to be uniformly distributed in the interval [0,1): this means that their \emph{id} is generated flipping a fair coin. This is a necessary condition for the collision tree to be balanced (the expected height of each leaf is the same) allowing to estimate the level to analyze using $\lfloor\log_{2}\hat{n}\rfloor$.\\ We remember also that the optimal probability $p$ that provides the best performance  for the MBT is $p=0.4175$. The problem is that with this $p$ the tree is no more balanced since the nodes are recursively splitted  in sets of expected different sizes: $np$ and $n(1-p)$.\\ 

The ideas are the following: 
\begin{itemize}
\item when a node realizes to belong to a sub-tree whose root has path $\mathbf{a}$, it must always remain in that sub-tree,
\item after a collision takes place in a node at level $\hat{N}=\lfloor\log_{2}\hat{n}\rfloor$, the nodes interested in the collision  gets redistributed in the collided interval.
\end{itemize}

Let each node $n_{i}$ is identified by a $L$-length binary string $\mathbf{b_{i}}=(b_{i1}b_{i2}\ldots b_{iL})$. Then, if applying the MBT we get a collision, each node interested by the collision updates its \emph{id} as follows: $\mathbf{b_{i}}=(b_{i1}b_{i2}\ldots, b_{i\hat{N}}c_{i1}\ldots c_{iL_{\hat{N}}})$ where $c_{i}k=0$ with probability $p=0.4175$ and $c_{i}k=1$ with probability $1-p$.\\  
...
Figure \ref{EBTvsEBTopt} shows the expected improvement in percentage of the ratio between the expected cost of EBT over  the expected cost of the modified EBT. It results oscillatory with mean 1.42.
...
In general, we assume not to have any \emph{a priori} statistical knowledge about the multiplicity of the nodes involved in a collision. Hence estimation techniques must provide efficient ways to obtain an estimate for the general zero-knowledge scenario.\\
\section{Clipped Binary Tree}
\label{cbt-estimation}
The most simple idea to obtain an estimate of the batch size could be to solve a minimum amount of nodes to obtain an estimate. This can be done, for example, by using deterministic algorithms such as \emph{Clipped Binary Tree} (CBT).\\
...
This is due to the fact that the batch of interest we use for the estimate becomes, at each level, smaller and smaller: the estimate, even for huge sizes, depends only on very few (3-5) nodes (those with lower addresses). Consequently estimate is quite innacurate.\\
