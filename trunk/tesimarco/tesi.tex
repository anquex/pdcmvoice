...
To obtain an upper bound on the expeted time as $ n \rightarrow \infty$ further analysis techniques has to be used but here we want simply focus on how the algorithm behaves when $n$ grows.\\
...
\node [circle,draw,label=below:\itshape C] (r){1}{
...
\subsubsection{Tree visit rules}
...
If $t$ is undefined then in $b_{1..k}$ there is no 0. This means that $b_{1..k} = \epsilon$ or $b_{1..k}=\underbrace{1\ldots 1}_{k}$ in both cases this indicates all the three has been explored.\\ So this work also for termination condition.\\
