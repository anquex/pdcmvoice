...
By applying Chebychev's Inequality and remebering $\hat{n}=j/p_{\epsilon}$ we can establish a relation between the relative error in the estimate and it's probability.  
 
 \begin{equation}
P\left( \left| \hat{n}-np_{\epsilon}\right| \geq \epsilon n \,|\, n \right) \leq \frac{1-p_{\epsilon}}{\epsilon^{2}n}
 \end{equation}
...
\section{Cidon Normal Approximation}
We remember from section \ref{cidon-estimate-accuracy} that
\begin{equation}P(J=j|n)={n \choose j}p_{\epsilon}^{j}(1-p_{\epsilon})^{n-j}\end{equation}
and
...
\begin{equation}
solve \left(P(j \leq knp ) - P(j < \frac{np}{k}) = 0.99 \right) \to p_{\epsilon}
\end{equation}
