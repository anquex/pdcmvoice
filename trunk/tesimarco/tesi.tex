...
In the original paper \cite{cidon} there is no detailed analysis of the behaviour of the algorithm but it is only shown the following fact: as $n$ grows the estimator becomes more accurate.\\
...
\item  \begin{equation}
P\left( \left| \frac{\hat{N}}{n}-1\right| \geq \epsilon  \,|\, n \right) \leq \frac{1-\pc}{\epsilon^{2}n}
\label{eq:cidon-cheby}
 \end{equation}
\end{enumerate}
\clearpage
...
In general $\phi$ and $\Phi$ depend on the size of the problem. We present Table \ref{table:phi-Phi} to provide an idea of the behaviour of $\phi$ as function of $n$.\\
...
Using basic Algorithm \ref{alg-greenberg}, even though the expected value of $\hat{n}_{+}$ is quite close to $n$, $\hat{n}_{+}$ is likely to difer from $n$ by a factor of 2. In the original work a small generalization of the base 2 algorithm is proposed to remedy this limit: providing and estimate whose mean is  close to $n$ but whose  distribution peaks more sharply about the mean.\\
...
\caption{\emph{Cidon}:  estimate accuracy dramatically improves while $\pc\leq0.1$.}
\end{center}
\end{figure}
...
\caption{\emph{Cidon}: Minimum $\pc$ required for accuracy $k$ with $\theta=0.99$. $k$ step is 0.02}
...
\caption{\emph{Cidon}: Upper bounds on the expected time (in slots) required to achieve accuracy $k$ with $\theta=0.99$. $k$ step is 0.02}
