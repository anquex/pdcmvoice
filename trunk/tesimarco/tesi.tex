...
\begin{equation}
\textrm{Pr}(\hat{n}=n'|n)=\sum_{l}f(n',l)\sum_{i}\sum_{s} \hat{X}_{MLE(i,s)}(n') \cdot f_{T}(i,s,c,p(l),n'),
\end{equation}
 where  $\hat{X}_{MLE(i,s)}(n')=1$ iff $MLE(i,s,c,l)=n'$,  $\hat{X}_{MLE(i,s)}(n')=0$ otherwise. 
