...
\caption{\emph{EGA} with $T=20$ :  large $2^{k}$ sizes distribution.}
...
Looking at the Table \ref{tb:GEGA-T10-l10} emerges that the estimate is always finite. This good propriety  tfollows from the bounded moments of the Greenberg algorithm. In general (see Table \ref{tb:window-estimate}), simple window based MLE estimators can not achieve finite estimate when all the events are collisions.\\
...
\makebox[\textwidth][c]{% exchange [l] with [r] or [c] to extend it into the left border or both, respectively
  \includegraphics[width=1.25\textwidth]{matlab/GEGA/GEGA-bias-varying-T}% here 20% wider than \textwidth
 }
\caption[\emph{GEGA}: Estimate Bias]{\emph{GEGA}: Estimate Bias. The tree plots show the estimate expected value over the real batch size for different $T$. Data reported were computed using formula \eqref{eq:EGA expected estimate given n}.}
\label{fg:GEGA-bias}
\end{figure}
