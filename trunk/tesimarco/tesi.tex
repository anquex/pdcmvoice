...
\end{equation}
which reveals that as the number of resolved terminals increases, increasing the value of $p$ ,the estimation $\hat{n}$ becomes more accurate.
%%%%%%%%%%%%%%%%%%%%%
\begin{comment}
\begin{equation}
\begin{split}
...
\end{itemize}
In Figure \ref{fg:comparison} we try to answer the question: ``after time $t$ what is the accuracy of the estimate?''.
\begin{figure}[htbp]
\centering
...
\label{fg:comparison}
\end{figure}
We considered the average running time for both the algorithms to provide a meaningful ad fair comparison: in \emph{GEGA} $t\approx\log(n)+T$, in \emph{Cidon} $t\approx n\pc C$.   Figure \ref{fg:comparison} shows the minimum  $k$ for which $\theta_{k}$ in \eqref{eq:zanella-bound} is greater than or equal to 0.99.
You can notice that the \emph{GEGA} algorithm, since it provides always a finite estimate, gives a bound on the accuracy after just  $\log(n)$ slots: as soon as phase 1 ends. It results faster than \emph{Cidon} in providing the first bound for any batch size. Considering $n=10$ in the figure as a index of the behavior for very small batch sizes, you can see that initially \gega estimate is more accurate than \cidon but for $k$ less than two \cidon takes less time. For larger sizes ($n=100$ and $n=1000$) \gega achieves accuracy $k=2$ in about the half of the time taken by \cidon, and $k=1.5$ saving about $40-45$\% of time.\\
