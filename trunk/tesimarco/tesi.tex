...
The most simple idea to obtain an estimate of the batch size is to solve a minimum amount of nodes. This can be done, for example, by using deterministic algorithms such as \emph{Clipped Binary Tree} (CBT).\\
...
When the algorithm stops we know than the last two nodes resolved owns to the same level $i$ of the tree (root is assumed to be at level 0).\\
We can obtain an estimate as:
\begin{equation}
\hat{n}\gets2^{i}
\end{equation}
\begin{comment}
...
\begin{enumerate}
\item Estimating of the initial batch using a partial deterministic resolution scheme. 
\item Performing an optimized complete deterministic resolution based on the results of phase 1. 
\end{enumerate}

The strategy adopted to obtain the estimate is to resolve a small part of the batch and to accumulate the number of successful transmission resulted.\\
