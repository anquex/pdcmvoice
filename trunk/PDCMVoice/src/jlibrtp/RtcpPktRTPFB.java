...
public class RtcpPktRTPFB extends RtcpPkt {
    ...
    private static final Logger LOGGER =
        Logger.getLogger(RtcpPktRTPFB.class.getName());
    ...
    protected RtcpPktRTPFB(byte[] aRawPkt, int start, RTPSession rtpSession) {
        ...
        if(! super.parseHeaders(start) || packetType != 205 || super.length < 2) {
            if(LOGGER.isLoggable(Level.FINEST)) {
                LOGGER.finest(" <-> RtcpPktRTPFB.parseHeaders() etc. problem");
            }
            super.problem = -205;
        } else {
            //FMT = super.itemCount;
    ...
    protected void debugPrint() {
        LOGGER.finest("->RtcpPktRTPFB.debugPrint() ");
        LOGGER.finest("  ssrcPacketSender: " + super.ssrc + "  ssrcMediaSource: " + ssrcMediaSource);

        if(this.PID != null && this.PID.length < 1) {
            LOGGER.finest("  No Feedback Control Information (FCI) fields");
        }

        for(int i=0; i < this.PID.length; i++ ) {
            LOGGER.finest("  FCI -> PID: " + PID[i] + "  BLP: " + BLP[i]);
        }
        LOGGER.finest("<-RtcpPktRTPFB.debugPrint() ");
    }
}
