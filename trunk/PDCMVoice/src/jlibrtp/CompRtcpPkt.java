...
public class CompRtcpPkt {
    ...
    protected void addPacket(RtcpPkt aPkt) {
        if(LOGGER.isLoggable(Level.FINE)) {
            LOGGER.fine("  <-> CompRtcpPkt.addPacket( "+ aPkt.getClass() + " )");
        }
    ...
    protected CompRtcpPkt(byte[] rawPkt, int packetSize, InetSocketAddress adr, RTPSession rtpSession) {
        if(LOGGER.isLoggable(Level.FINE)) {
            LOGGER.fine("-> CompRtcpPkt(" + rawPkt.getClass() + ", size " + packetSize + ", from " + adr.toString() + ", " + rtpSession.getClass() + ")");
        }
        ...
        while(start < packetSize && problem == 0) {
            ...
            if(start == 0) {
                ...
                if(pktType != 200 && pktType != 201 ) {
                    LOGGER.info("!!!! CompRtcpPkt(rawPkt...) packet did not start with SR or RR");
                    this.problem = -1;
                }

                // Padding bit should be zero for the first packet
                if(((rawPkt[start] & 0x20) >>> 5) == 1) {
                    LOGGER.info("!!!! CompRtcpPkt(rawPkt...) first packet was padded");
                    this.problem = -2;
                }
            }
            ...
            if(pktType == 200) {
                ...
                LOGGER.warning("!!!! CompRtcpPkt(byte[] rawPkt, int packetSize...) "
            ...
            if(LOGGER.isLoggable(Level.FINER)) {
                LOGGER.finer(" start:"+start+"  parsing pktType "+pktType+" length: "+length);
            }
        }
        if(LOGGER.isLoggable(Level.FINE)) {
            LOGGER.fine("<- CompRtcpPkt(rawPkt....)");
        }
    }
    ...
    protected byte[] encode() {
        if(LOGGER.isLoggable(Level.FINER)) {
            LOGGER.finer(" <- CompRtcpPkt.encode()");
        }
