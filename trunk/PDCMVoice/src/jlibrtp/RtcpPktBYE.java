...
public class RtcpPktBYE extends RtcpPkt {
    ...
    protected RtcpPktBYE(long[] ssrcs,byte[] aReason) {
        ...
        ssrcArray = ssrcs;
        if(ssrcs.length < 1) {
            LOGGER.warning("RtcpBYE.RtcpPktBYE(long[] ssrcs, byte[] aReason) requires at least one SSRC!");
        }
    }
    ...
    protected RtcpPktBYE(byte[] aRawPkt, int start) {
        ...
        if(!super.parseHeaders(start) || packetType != 203 ) {
            if(LOGGER.isLoggable(Level.FINEST)) {
                LOGGER.finest(" <-> RtcpPktBYE.parseHeaders() etc. problem");
            }
            super.problem = -203;
        } else {
    ...
    public void debugPrint() {
        ...
        if(ssrcArray != null) {
            for(int i= 0; i<ssrcArray.length; i++) {
                ...
                LOGGER.finest("     ssrc: " + anSsrc);
            }
        }
        if(reason != null) {
            LOGGER.finest("     Reason: " + new String(reason));
        }
    }
}
