...
public class RtcpPktSR extends RtcpPkt {
    ...
    private static final Logger LOGGER =
        Logger.getLogger(RtcpPktSR.class.getName());
    ...
    protected RtcpPktSR(byte[] aRawPkt, int start, int length) {
        if(LOGGER.isLoggable(Level.FINEST)) {
            LOGGER.finest("  -> RtcpPktSR(rawPkt)");
        }
        ...
        if(!super.parseHeaders(start) || packetType != 200 ) {
            if(LOGGER.isLoggable(Level.FINEST)) {
                LOGGER.finest(" <-> RtcpPktSR.parseHeaders() etc. problem: "+ (!super.parseHeaders(start) ) + " " + packetType + " " + super.length);
            }
            super.problem = -200;
        } else {
    ...
    protected void encode() {
        if(LOGGER.isLoggable(Level.FINEST)) {
            if(this.rReports != null) {
                ...
                LOGGER.finest("  -> RtcpPktSR.encode() receptionReports: null");
            }
        }
        ...
        if(LOGGER.isLoggable(Level.FINEST)) {
            LOGGER.finest("  <- RtcpPktSR.encode() ntpTs1: "
    ...
    public void debugPrint() {
        LOGGER.finest("RtcpPktSR.debugPrint() ");
        LOGGER.finest("  SSRC:"+Long.toString(super.ssrc) +" ntpTs1:"+Long.toString(ntpTs1)
        ...
        if(this.rReports != null) {
            ...
            LOGGER.finest("No Receiver Reports associated with this Sender Report.");
        }
    }
}
