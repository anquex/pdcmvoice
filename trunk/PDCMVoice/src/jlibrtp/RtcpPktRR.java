...
public class RtcpPktRR extends RtcpPkt {
    ...
    private static final Logger LOGGER =
        Logger.getLogger(RtcpPktRR.class.getName());
    ...
    protected RtcpPktRR(byte[] aRawPkt, int start, int rrCount) {
        ...
        if(rrCount < 0 && (!super.parseHeaders(start) || packetType != 201 || super.length < 1)) {
            if(LOGGER.isLoggable(Level.FINEST)) {
                LOGGER.finest(" <-> RtcpPktRR.parseHeaders() etc. problem: "+(!super.parseHeaders(start))+" "+packetType+" "+super.length);
            }
            super.problem = -201;
        }
    ...
    protected void encode() {
        if(LOGGER.isLoggable(Level.FINEST)) {
            LOGGER.finest("  -> RtcpPktRR.encode()");
        }
    ...
    protected byte[] encodeRR() {
        if(LOGGER.isLoggable(Level.FINEST)) {
            LOGGER.finest("   -> RtcpPktRR.encodeRR()");
        }
    ...
    public void debugPrint() {
        LOGGER.finest("RtcpPktRR.debugPrint() ");
        if(reportees != null) {
            for(int i= 0; i<reportees.length; i++) {
                Participant part = reportees[i];
                LOGGER.finest("     part.ssrc: " + part.ssrc + "  part.cname: " + part.cname);
            }
        } else {
            for(int i=0;i<reporteeSsrc.length; i++) {
                LOGGER.finest("     reporteeSSRC: " + reporteeSsrc[i] + "  timeStampLSR: " + timeStampLSR[i]);
            }
        }
    }
}
