...
L'applicazione della FFT non velocizza il calcolo della trasformata discreta di Fourier ma riduce al contempo la propagazione dell'errore di arrotondamento.
...
\midrule
Direct Call & $O(\log_2N)$\\
Repeated multiplication & $O(N\log_2N)$\\
Subvector Scaling & $O((\log_2N)^2)$\\
Forward Recursion & $O(N^2\log_2N)$\\
\bottomrule
\end {tabular}
\end{center}
\end{table}
