...
&=&X_k
\end{eqnarray}
dove si è utilizzato il fatto che $e^{-\jmath 2\pi }=1$. Allo stesso modo pu\`o essere mostrato che la \ref{def_idft} porta a un'estensione periodica della IDFT.
...
\begin{gather}
\nonumber \text{Se}\qquad\qquad x_n \longleftrightarrow X_k\\
\text{Allora} \qquad x_ne^{\jmath\frac{2\pi}{N}nm}\longleftrightarrow X_{k-m}\\
\text{e}\qquad x_{n-m}\longleftrightarrow X_ke^{-\jmath\frac{2\pi}{N}km}
\end{gather}
Un caso molto utile, soprattutto nelle sue applicazioni $d$-dimensionali è
\begin{gather}
\nonumber x_n \longleftrightarrow X_k\\
	x_n\cdot(-1)^n= x_n\cdot e^{j2\pi(\frac{N/2n}{N})} \longleftrightarrow X_{k-\frac{N}{2}}
	\label{eq:frequency-shift}
\end{gather}
che ha l'effetto di centrare le basse frequenze.
...
\end{equation}
\\
Matricialmente la convoluzione ciclica è espressa come
\begin{align}
\begin{pmatrix}\omega_0\\ \omega_1 \\\omega_2 \\ \vdots \\ \omega_{N-1} \end{pmatrix}&=\begin{pmatrix}y_0 & y_{N-1} & y_{N-2} & \cdots & y_{1}\\ y_{1} & y_0 & y_{N-1} &\cdots & y_{2}\\y_{2} & y_1 & y_{0} &\cdots & y_{3}\\ \vdots &\vdots &\vdots &\ddots &\vdots\\y_{N-1} & y_{N-2} & y_{N-3} & \cdots & y_{0}
\end{pmatrix}\times\begin{pmatrix} x_0 \\ x_1 \\ x_2 \\ \vdots\\ x_{N-1} \end{pmatrix}\\
\omega &= C(y) \times x
\end{align}
...
\end{equation}
\`e una \emph{radice complessa N-esima dell'unità}.\\
...
\begin{pmatrix}
	...
	\left\langle F^{N-2}, F^0\right\rangle & \left\langle F^{N-2}, F^1\right\rangle &\left\langle F^{N-2}, F^2\right\rangle & \cdots & \left\langle F^{N-2}, F^{N-1}\right\rangle\\
...
\end{pmatrix} = N\cdot \mathbf{I}
\end{align}
Questo ci è utile per definire le versioni normalizzate  della DFT/IDFT con matrici unitarie
\begin{equation}
	\widetilde{F}_N=\frac{1}{\sqrt{N}} F_N  \qquad \qquad  \widetilde{F}_N^{-1}=\frac{1}{\sqrt{N}} F_N^*
\end{equation}
...
\section{Segnali di durata finita: relazione tra DTFT e DFT}
...
La DFT costituisce un valido metodo per il calcolo della DTFT di una sequenza finita, tanto più in quanto sono disponibili algoritmi efficienti (FFT) per la sua determinazione.
...
\begin{cases} 
1 \quad \text{per} \quad k = 0 \\
0 \quad \text{per} \quad 1 \leq k \leq N-1
\end{cases}\]
\\
e di conseguenza
\\
...
\end{equation*}
\\
In definitiva è possibile ottenere $X(e^{j\omega})$ al variare di $\omega$ in $\mathbb{R}$ considerando gli elementi della DFT $X_k$ come i valori di $X(e^{j\omega})$ in $\omega = 2 \pi k / N$, $0 \leq k \leq N-1$ e considerando, per gli altri valori di $\omega$, il risultato dell'interpolazione \eqref{eq:dft-interpolation-3}.

\chapter{Calcolo della DFT}

La diffusione nell'utilizzo della DFT è principalmente dovuta all'introduzione, avvenuta negli anni `60, di una classe di algoritmi computazionale efficienti per la sua valutazione: la \emph{Fast Fourier Transform (trasformata veloce di Fourier)}.\\ Essa rende possibile in taluni casi l'elaborazione in tempo reale (sistemi di navigazione, trasmissione e processo di segnali vocali o televisivi) e consente di risolvere alcuni problemi di grandi dimensioni che risulterebbero altrimenti intrattabili.\\
L'FFT viene normalmente attribuita a Cooley e Tukey (`65). Di fatto questo particolare algoritmo venne scoperto (ed usato) da Gauss nel XIX secolo. Pare che in realtà l'algoritmo sia stato riscoperto ed utilizzato varie volte (anche prima del lavoro di Cooley e Tukey sopra menzionato).
...
\mathcal{F} : \mathbb{C}^N\longrightarrow\mathbb{C}^N
\end{equation*}
ci sono $N$ termini da calcolare, ognuno dei quali richiede $N$ moltiplicazioni e $N-1$ addizioni complesse. Anche evitando di calcolare le moltiplicazioni per $\omega_{N}^{0}=1$ si devono comunque effettuare in totale $2(N-1)^2$ operazioni complesse. Il costo è pertanto $O(N^2)$

Nel dettaglio ogni moltiplicazione complessa $\lambda = z_1 \cdot z_2 = (a + jb) \cdot (c + jd)$ si sviluppa in 
$$ \lambda= (a \cdot c - b \cdot d)+ j (a \cdot d + b \cdot c)$$
oppure in
\begin{align*}
m1 &= (a + b) \cdot c\\
m2 &= (d + c) \cdot b\\
...
\rm{Im}(\lambda) &= m1 + m3\\
\end{align*}
richiedendo  rispettivamente 4 moltiplicazioni reali e 2 addizioni nel primo caso e 3 moltiplicazione e 5 addizioni nel secondo.\\
Il numero di operazioni macchina totali risultante nel primo caso è $4(N-1)^2$ moltiplicazioni e $2N(N-1)+2(N-1)^2= 2(N-1)(2N-1)$ addizioni.

Il secondo caso può risultare utile in particolari situazioni in quanto, poichè nel calcolo della FFT tutte le moltiplicazioni complesse coinvolgono un "`\emph{twiddle factor}"' $\omega_N^l = c + jd$ e un ulteriore termine, è possibile, oltre che precalcolarli, organizzare le operazioni in modo da salvarsi anche i risultati intermedi $c+d$ e $c-d$. Con questa accortezza una moltiplicazione complessa richede sempre 6 operazioni totali di cui soltanto 3 sono moltiplicazioni. 
...
\end{equation}
Ovviamente, minore è la complessità originaria (quindi l'esponente $\alpha$) e più piccolo sarà il tempo di calcolo al quale si arriva applicando il principio \emph{"`divide et impera"'}; in ogni caso, comunque, esso sarà dell'ordine di $N \cdot \log_2 N$

Osserviamo inoltre che il principio \emph{"`divide et impera"'} conduce spontaneamente a strutture di calcolo parallele. Con $N=2^\nu$, le partizioni dei dati più efficienti sono le più "`spontanee"', e sono "`schematizzate"' nella figura seguente per $N=8$
\vskip 30pt
\begin{figure}[h!]
  \centering
...
Dato un vettore $\mathbf{x}\in{}\mathbb{C}^n$, l'algoritmo di Cooley-Tukey  si articola in cinque fasi:
\begin{itemize}
\item Fase 1: il vettore di ingresso viene riorganizzato in forma matriciale $p \times q$ secondo un ordinamento row-major. Tipicamente o $p$ o $q$ \`e un fattore piccolo, non necessariamente primo, che \`e chiamato radice.
...
Per $p=2$ si ottiene un algoritmo radix-2 nel tempo, ovvero che risolve il problema originale scomponendolo in due sottoproblemi di taglia dimezzata su campioni alterni dell'input. Lo schema di scomposizione risultante è il seguente.
...
\begin{equation}
Z_r=\sum_{k=0}^{\frac{N}{4}-1}{x_{4k+1}\omega_{N}^{r(4k)}} =\sum_{k=0}^{\frac{N}{4}-1}{x_{4k+1}{(\omega_{N}^{4})}^{rk}} =\sum_{k=0}^{\frac{N}{4}-1}{z_{k}\omega_{\frac{N}{4}}^{rk}}, \qquad r=0,1,... ,N/4-1
\end{equation}

\begin{equation}
H_r=\sum_{k=0}^{\frac{N}{4}-1}{x_{4k+3}\omega_{N}^{r(4k)}} =\sum_{k=0}^{\frac{N}{4}-1}{x_{4k+3}{(\omega_{N}^{4})}^{rk}} =\sum_{k=0}^{\frac{N}{4}-1}{h_{k}\omega_{\frac{N}{4}}^{rk}}, \qquad r=0,1,... ,N/4-1
\end{equation}
...
\begin{align*}
	...
	&= \omega_N^{-kN} \sum_{n=0}^{N-1} x[n] \omega_N^{nk} = \sum_{r=0}^{N-1} x[r] \omega_N^{-k(N-r)}\\
	\left.y_k[n]\right|_{n=N}&= \left.\left[x[n]\ast \left( \omega_N^{-nk} u[n]\right)\right]\right|_{n=N}
\end{align*}
che è una convoluzione discreta\footnote{
La convoluzione discreta è definita come: $\displaystyle
y(n)=x(n)\ast h(n)= \sum_{r=-\infty}^{+\infty}x(r)h(n-r)
$
}.
\begin{equation*}
...
$$
X_{k}=(((\omega_N^{-k}x[0]+x[1])\omega_N^{-k}+x[2])\omega_N^{-k}+\dots+x[n-1])\omega_N^{-k}
$$
e l'algoritmo, espresso come equazione alle differenze del primo ordine, fornisce
\begin{align*}
...
Il calcolo di $X_k$ richiede tempo $O(N)$. Quindi per calcolare tutta la DFT si ha complessità $O(N^2)$. Tuttavia se si è interessati solo a $M$ frequenze risulta vantaggiosa rispetto agli algoritmi FFT $O(N \log_2N)$ fin tanto che $ M\leq\log_2N$
%
...
\end{equation}
dove $\mathbf{I_{m}}$ è la matrice identità di ordine \emph{m}$=8$ e $\mathbf{\Omega_{m}}$ è la matrice diagnonale 
\begin{equation*}
...
\end{split}
\end{align}
Quindi la matrice $\mathbf{F_{16}}$ è stata espressa come prodotto di quattro matrici contenenti $\mathbf{B}_m$ con $m=2,4,8,16$ e ad ogni passo la matrice  $\mathbf{B}_m$ implementa le relazioni di scambi e calcolo per passare da 2 DFT di tagllia $m/2$ a una di taglia $m$.\\
...
Dove un riordinamento di mensione N può essere espresso come una matrice $\mathbf{R_N}$  $N \times N$ del tipo
...
La fattorizzazione \eqref{eq:CT-factorization}  può inoltre essere espressa agevolmente attraverso  il \emph{Prodotto di Kronecker}
...
$$
dove$\mathbf{A}_{k}=\mathbf{I}_r\otimes\mathbf{B}_m $, $m=2^k$ e $r=2^{M-k}$ per $k=1,...,M$ sono matrici diagnonali a blocchi.
...
\begin{figure}[H]
  \centering
      \includegraphics[width=\textwidth]{immagini/CT-FFT-fasi}
  \caption{Operazioni effettuate dall'algoritmo di CT per N=8. Si noti la fase iniziale di riordino e la successiva di calcolo}
\end{figure}
...
I dati di ingresso (elementi del vettore $\mathbf{X}$) sono costituiti da diversi valori, ognuno dei quali è composto dalla sua parte reale e dalla parte immaginaria. Il processo 0 \`e il processo che si occupa sempre di leggere i valori di ingresso e salvarli opportunamente in un array \emph{double} di dimensione doppia rispetto alla dimensione del vettore $\mathbf{X}$, dato che la sistemazione dei valori all'interno dell'array avviene in maniera tale che l'elemento i-esimo abbia la sua parte reale salvata in posizione $2*i$ e la parte immaginaria in posizione $2*i+1$ nell'array di input.\\Successivamente, completata l'acquisizione, il processo 0 esegue un riordinamento \emph{bit-reversing} dei dati sulle linee ed infine esegue un broadcast dell'intero contenuto dell'array verso i rimanenti processi.
...
Fatto ci\`o la computazione vera e propria ha inizio: essa si compone di $log_2(n)$ passi dove ogni passo coincide con un ``settore'' della rete ascendente. All'inizio di ogni passo ogni processo, se necessario, prima di effettuare qualunque calcolo esegue una primitiva non bloccante (come \verb+MPI_Irecv+) un numero di volte pari al numero di linee che gestisce al fine di preparare il buffer di ricezione per i dati che verranno ricevuti dai dovuti \emph{peers} al termine del passo. Conseguentemente, al termine di ogni passo e prima dell'inizio del successivo, se necessario ogni processo invia il/i dato/i calcolato/i durante il passo appena terminato al peer opportuno utilizzando una primitiva non bloccante (come \verb+MPI_Isend+). Prima del passaggio al passo successivo ogni processo attende il completamento delle operazioni di ricezione dati avviate all'inizio del passo precedente, dato che ovviamente tali dati saranno necessari per le prossime fasi.\\Al termine della computazione, infine, ogni processo invia al processo 0 i dati appartenenti alle linee di propria competenza affinch\`e vengano posti in un array comune ed infine visualizzati.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{immagini/processi} 
  \caption{aree di responabilità per 4 processori prima e dopo la trasposizione}           
\end{figure}
...
dove $u\approx 2^{-b}$ con $b$ lunghezza della mantissa.
...
\noindent Si suppone inoltre che gli esponenziali complessi $\omega_k$ siano pre-calcolati e su di essi sia stato commesso un errore $|\delta_{jk}|$ tale che:

\begin{equation*}
\hat{\omega}_j^k= fl(\omega_j^k)=\omega_j^k+\delta_{jk} \qquad |\delta_{jk}|\leq u
\end{equation*}
...
Se si utilizza la definizione di DFT per calcolare la trasformata di Fourier discreta $\mathbf{X}$ di una sequenza $\mathbf{x}$ di lunghezza $N$ in aritmetica a virgola mobile a lunghezza di parola finita e mantissa di $b$ bit, Allora
\begin{equation}
...
\end{equation}
$\mathbf{x}$ può essere visto come $\mathbf{x}={\rm Re}(\mathbf{x})+j{\rm Im}(\mathbf{x}) \leftrightarrow [{\rm Re}(\mathbf{x}), {\rm Im}(\mathbf{x})]$, vettore di lunghezza $2N$. Per definizione
\begin{align}
X_k&=\sum_{n=0}^{N-1}x_ne^{-\jmath\frac{2\pi}{N}kn} \quad k=0,...,N-1\\
...
\begin{align*}
\Phi\Phi^T= N \cdot \underbrace{\begin{bmatrix}
1 & 0 & ... & 0\\ 0 & 1 & ... & 0\\ \vdots & \vdots & \ddots & \vdots\\ 0 & 0 & ... & 1\\
\end{bmatrix}}_{2N} \longrightarrow \frac{1}{\sqrt{N}}\Phi \qquad \text{è ortonogonale}
\end{align*}

\begin{equation}
\mathbf{X} = \Phi \mathbf{x} \longrightarrow \left\|\mathbf{X}\right\|_2=\sqrt{N} \left\|\mathbf{x}\right\|_2
\end{equation}
...
\emph{Proprietà} La matrice $\dfrac{1}{\sqrt{2}}\mathbf{B}_{2m}$ è unitaria. Infatti
...
\noindent Ovvero
\begin{equation*}
	\frac{\left\|\Delta X\right\|_2}{\left\|X\right\|_2}=O(log_2N \;c_Nu)
\end{equation*}
\end{Teorema2}
...
\begin{equation}
	\hat{\mathbf{x}}_j=\hat{\mathbf{A}}_j\mathbf{\hat{x}}_{j-1}+\mathbf{e}_{j}\qquad(j=1,...,n)
	\label{eq:3.7}
\end{equation}
...
\begin{enumerate}
	...
	$$
	Questo passo pertanto si costituisce di $N/2$ \emph{operazioni butterfly} su due elementi. I twiddle factor risultano banali e le operazioni si riducono a 
	...
	\begin{align*}
	\hat{\eta}_1 &= (\xi_1 +\xi_2)(1+\varepsilon_1^+)=\eta_1+(\xi_1+\xi_2)\varepsilon_1^+ & (|\varepsilon_1^+|,|\varepsilon_2^+|\leq u)\\
	\hat{\eta}_2 &= (\xi_1 -\xi_2)(1+\varepsilon_2^+)=\eta_2+(\xi_1+\xi_2)\varepsilon_2^+
	\end{align*}
	da cui
...
\end{equation}
	...
	\item Passo $j=2$. Notiamo che
		...
		con
		$$\mathbf{B}_4=\begin{pmatrix}\mathbf{I}_2 &\mathbf{\Omega}_2\\\mathbf{I}_2 &-\mathbf{\Omega}_2 \end{pmatrix} \quad\mathbf{\Omega}_2=\begin{pmatrix}1 &0\\ 0& -i\end{pmatrix}\quad \text{e quindi}\quad \mathbf{B}_4=\begin{pmatrix}1&0&1&0\\0&1&0&-i\\1&0&-1&0\\0&1&0&i\end{pmatrix}$$
		Il secondo passo consiste quindi in $N/4$ \emph{operazioni butterfly} di 4 elementi. I twiddle factor sono banali e le operazioni risultano essere $(\xi_1,\xi_2,\xi_3,\xi_4\in \mathbb{C})$
			\begin{align*}
			\eta_1 &= \xi_1+\xi_3 &
			\eta_2 &= \xi_2-i\xi_4\\
			\eta_3 &= \xi_1-\xi_3&
			\eta_4 &= \xi_2+i\xi_4
	\end{align*}
	Procedendo analogamente al caso precedente si ottiene la stima $\left\|\mathbf{e}_2\right\|_2\leq\sqrt{2}u \left\|\mathbf{\hat{x}}_1\right\|_2$.
	Per la \eqref{eq:3.7} e la \eqref{eq:3.9} otteniamo che 
	\begin{equation*}
	\left\|\hat{\mathbf{x}}_1\right\|_2\leq \underbrace{\left\|\mathbf{A}_1\right\|_2}_{\sqrt{2}}\left\|\hat{\mathbf{x}}_0\right\|_2+\underbrace{\left\|\mathbf{e}_1\right\|_2}_{\leq \sqrt{2} u \left\|\hat{\mathbf{x}}_0\right\|_2}\leq \sqrt{2} (1+u)\left\|\mathbf{x}\right\|_2
\end{equation*}
e quindi
\begin{equation}
...
\end{equation}
	...
	Osserviamo che la matrice $\mathbf{A}_j$ presenta per ogni riga e colonna esattamente due elementi diversi da zero. In ogni riga inoltre un elemento è $1$ mentre l'altro è un generico \emph{twiddle factor}. Al passo $j$ pertanto vengono effettuate $2^{n-j}$  \emph{operazioni butterfly}, ciascuna costituita da $2^{j-1}$ coppie di operazioni del tipo 
	\begin{align}
	\label{eq:3.11}
	\begin{split}
...
\begin{align}
\begin{split}
		\hat{\eta}_1 &= (\xi_1+\hat{\omega}_{2^j}^k\xi_2(1+\varepsilon^\times))(1+\varepsilon_1^+) \\
		\hat{\eta}_2 &= (\xi_1-\hat{\omega}_{2^j}^k\xi_2(1+\varepsilon^\times))(1+\varepsilon_2^+)
		\label{eq:3.13}
\end{split}
\end{align}
con
\begin{equation}
	...
	\label{eq:3.14}
\end{equation}	
Dall'ipotesi \eqref{eq:3.12} sui \emph{twiddle factor} e dalla \eqref{eq:3.13} segue che
$$
\hat{\eta}_1-\eta_1=\xi_1\varepsilon_1^++\hat{\omega}_{2^j}^k\xi_2(\varepsilon^\times+\varepsilon_1^++\varepsilon^\times \varepsilon_1^+)+\delta_{2^j}^k \xi_2+O(u^2)\
$$
e usando la \eqref{eq:3.14} si ha
$$
...
Pertanto, per l'errore $\mathbf{e}_j$ otteniamo la stima
$$\left\|\mathbf{e}_j\right\|_2\leq 2((2\sqrt{2}+1+c_N)u+O(u^2))\left\|\mathbf{\hat   {x}}_{j-1}\right\|_2$$

Dalla \eqref{eq:3.4} e \eqref{eq:3.7} segue che
$$
...
\begin{equation}\nonumber
 \begin{split}						  
	...
	\end{split}
\end {equation}
Dall'algoritmo \ref{alg:ct-matrix} (CT-Matriciale) ricordiamo che $\mathbf{x}_n = \mathbf{A}_n ...\mathbf{A}_1\mathbf{P}^T\mathbf{x}$ e per le \eqref{eq:3.4},\eqref{eq:3.9},\eqref{eq:3.10},\eqref{eq:3.15}
...
L'applicazione della FFT non solo velocizza il calcolo della trasformata discreta di Fourier ma riduce al contempo la propagazione dell'errore di arrotondamento.
Notiamo come per la DFT questo sia più che lineare mentre per la FFT risulta logaritmico e quindi praticamente costante per taglie di problemi molto grandi.
...
La verifica su dati reali mostra come bound DFT ed errore effettivo abbiano lo stesso andamento asintotico a meno di una costante quantificabile in circa un ordine di grandezza.
...
Notiamo come l'errore reale e quello di bound per la FFT è circa 2 ordini di grandezza inferiore. La costante asintotica della FFT è circa $1/10$ di quella della DFT
...
In questo caso $\hat{\omega}_N^j$ è il  risultato di $j-1$ moltiplicazioni complesse e pertanto si ha che 
\begin{equation}
	|\epsilon_{j}| = O(ju)
\end{equation}
...
\end {align*}
che comporta $2\log_2N$ valutazioni trigonometriche e $N$  moltiplicazioni complesse
...
Questo metodo è presentato per completezza come esempio di metodo numericamente instabile. Il metodo discende dalle identità trigonometriche:
\begin{align}
...
In questo caso la propagazione dell'errore è dovuta principalmente alla moltiplicazione per $|\tau|=|2\cos(\theta)|$ che per $N\geq 6$ risulta già maggiore di 1.
\begin{equation}
	|\epsilon_{j}| = O(2j^2u)
\end{equation}
...
Forward Recursion & $O(2j^2u)$\\
\bottomrule
\end {tabular}
\end{center}
\end{table}

Con i vari metodi quindi, l'errore nel calcolo della FFT diventa

\begin{table}
\begin{center}
\begin{tabular}{ll}
\toprule
\multicolumn{1}{c}{\emph {Metodo}} & \multicolumn{1}{c}{\emph{Errore }}  \\
\midrule
Direct Call & $O(\log_2N\;u)$\\
Repeated multiplication & $O(N\log_2N\;u)$\\
Subvector Scaling & $O((\log_2N)^2\;u)$\\
Forward Recursion & $O(N^2\log_2N\;u)$\\
\bottomrule
\end {tabular}
\end{center}
\end{table}
...
Sono stati ottenuti generando in matlab un vettore casuale di lughezza $N=2^k$ con distribuzione uniforme e valori compresi tra 0 e 1. A questi vettori è stata quindi applicata FFT e IFFT con i vari metodi e quindi calcolato l'errore relativo
\\
\begin{equation*}
 \frac{\left\|\text{IFFT}(\text{FFT}(x)) \right\|}{\left\|x\right\|}
\end{equation*}
\\
...
L'errore tra DC-VS non è significativamente distinguibile mentre il numero di operazioni richieste da VS è decisamente inferiore. Infatti DC effettua un ordine lineare di valutazioni trigonometriche mentre VS soltanto un ordine logaritmico di queste e un ordine lineare di moltiplicazioni complesse.
...
Il metodo Forward Recursion è instabile ed il suo errore è maggiore di quello della DFT banale già per $N=2^6$. Il grafico compara l'errore con Forward Recursion, DFT con definizione e calcolo diretto.
\begin{figure}[H]
  \centering
      \includegraphics[width=\textwidth]{immagini/errore-fft_fr_dft}
  \caption{errore instabile}
\end{figure}
...
%
%
\section{DFT di una sequenza reale}

La DFT è una mappa $\displaystyle \mathcal{F} : \mathbb{C}^N\longrightarrow\mathbb{C}^N $ ma spesso nelle applicazioni si utilizzano 
esclusivamente dati reali come input. In questo caso, utilizzando opportunamente le simmetrie dei segnali e le loro trasformate è possibile ridurre ulteriormente la complessità di calcolo.


Dato un segnale $x[n]=g[n]+jh[n]$ e la sua trasformata $X[k]$ si ha che
\begin{align} \label{eq:dft-real-1}
x_{re}[n]=g[n] &\longleftrightarrow \frac{1}{2} \{X[k]+X^*[-k]\}= G[k]\\\label{eq:dft-real-2}
x_{im}[n]=h[n] &\longleftrightarrow \frac{1}{2j} \{X[k]-X^*[-k]\} = H[k]
\end{align}
dove le operazioni $[i]$ sono intese modulo $N$.\\
Se il vettore da elaborare è $v[n]\in\mathbb{R}^N$ possiamo considerarlo come
$$
g[n] =v[2n], \qquad h[n]=v[2n+1],\qquad 0\leq n \leq N/2-1
$$
pertanto
\begin{align}
V[k]&= \sum_{n=0}^{N-1} v[n] \omega_N^{nk} = \sum_{n=0}^{\frac{N}{2}-1} v[2n] \omega_N^{2nk}+\sum_{n=0}^{\frac{N}{2}-1} v[2n+1] \omega_N^{(2n+1)k}\\
&= \sum_{n=0}^{\frac{N}{2}-1} g[n] \omega_\frac{N}{2}^{nk}+\omega_N^k\sum_{n=0}^{\frac{N}{2}-1} h[n] \omega_\frac{N}{2}^{nk}\\
&=G[k]+\omega_N^k H[k]
\end{align}

Quindi risulta possibile calcolare la FFT di una sequenza reale di lunghezza $N$ attraverso una FFT di lunghezza $N/2$ con complessità $O(\frac{N}{2}log_2\frac{N}{2})$, un processing aggiuntivo di $O(N/2)$ operazioni per ricostruire $G[k]$ e $H[k]$ e un ulteriore $O(N)$ per ottenere $V[k]$.   

Poichè
$$O\left(\frac{N}{2}\log_2\frac{N}{2}+2N\right)\leq O\left(N\log_2N\right)\qquad \forall N \geq8$$
l'operazione risulta sempre vantaggiosa.
Notiamo inoltre che le \eqref{eq:dft-real-1} e \eqref{eq:dft-real-2} possono essere altresi utilizzate per il calcolo di 2 DFT reali di lunghezza N attraverso una unica FFT di lunghezza N
%
...
La N-point DFT $Y[k]$ della sequenza $y[n] = g[n] \circledast_N h[n]$ data dalla convoluzione circolare tra $g[n]$ e $h[n]$ è data da:
\begin{equation}
\label{appl_conv_circ}
	Y[k] = DFT(y[n])_k = DFT(g[n] \circledast_N h[n])_k = G[k]H[k]
\end{equation}
...
\begin{figure}[H]
  \centering
      \includegraphics[width=10cm,height=3cm]{immagini/circ-conv-mitra}
  \caption{Convoluzione circolare tramite FFT}
\end{figure}

\paragraph{Esempio}.\\
Date le due sequenze
$$g[n] = [1 2 0 1] \qquad \text{e}\qquad h[n] = [2 2 1 1]$$
per calcolare la convoluzione circolare
\begin{equation}
y_C[n] = g[n] \circledast_4 h[n] = \sum_{m=0}^{3} g[m]\cdot h[(n-m){\mod4}] \qquad 0 \leq n \leq 3
\end{equation}
...
\begin{align*}
  G[k] &= g[0] + g[1]e^{-j(\frac{2\pi}{4}1 \cdot k)} + g[2]e^{-j(\frac{2\pi}{4}2 \cdot k)} + g[3]e^{-j(\frac{2\pi}{4}3 \cdot k)}
  \\
  &= g[0] + g[1](-j)^k + g[2](-1)^k + g[3](-j)^{3k} 
  \\
       &= 1 + 2(-j)^k + (-j)^{3k} & 0 \leq k \leq 3
\end{align*}
ottenendo quindi per $k=0,1,2,3$
\begin{align*}
  G[0] &= 1 + 2 + 1 = 4 & G[1] &= 1 - j2 + j = 1-j\\
  G[2] &= 1 - 2 - 1 = -2& G[3] &= 1 + j2 - j = 1+j
\end{align*}
...
\begin{equation*}
	y_L[n] = g[n] \ast h[n]=\sum_{m=0}^{N+M-1}g[m]h[n-m]
\end{equation*}
...
\end{equation*}
dove $L = M + N - 1$.\\
La convoluzione lineare $y_L[n]$ di lunghezza corrisponde alla convoluzione circolare tra le sequenza estese:
\begin{equation*}
	y_L[n]=y_{Ce}[n]=g_e[n] \circledast_L h_e[n]
\end{equation*}

\begin{figure}[H]
  \centering
      \includegraphics[width=10cm,height=3cm]{immagini/lin-conv-mitra}
  \caption{Convoluzione lineare di due sequenza di lunghezza finita tramite FFT}
\end{figure}

Nel caso in cui una delle due sequenze sia di durata molto maggiore dell'altra (si può pensare ad una sequenza $x[n]$ di durata infinita e ad una sequenza $h[n]$ di lunghezza M) è possibile utilizzare un metodo detto Overlap-Add, che prevede di suddividere la sequenza di partenza $x[n]$ in più sequenze $x_m[n]$ di $N$ campioni non sovrapposti:
\begin{equation*}
...
\begin{equation*}
	y_L[n]= h[n] \ast x[n] = \sum_{l=0}^{M-1} h[l]x[n-l]
\end{equation*}
come
\begin{eqnarray*}
	y_L[n]&=& \sum_{l=0}^{M-1} h[l] \left( \sum_{m=0}^\infty x_m[n-l-mN] \right)
	    \\
...
\begin{equation*}
	y_m[n]= h[n] \ast x_m[n]
\end{equation*}
La convoluzione lineare $y_L[n]$ viene così suddivisa in più convoluzioni lineari $y_m[n]$ di lunghezza minore $(N + M - 1)$.
Per esempio $y_0[n]= h[n] \ast x_0[n]$ è definita per $0 \leq n \leq N+M-2$ e $y_1[n]= h[n] \ast x_1[n]$ è definita per $N \leq n \leq 2N+M-2$; la sovrapposizione tra gli $M-1$ elementi di $y_0[n]$ e $y_1[n]$ viene risolta sommando tali elementi per otterenere la sequenza definitiva $y_L[n]$.

\begin{figure}[H]
  \centering
      \includegraphics[width=10cm,height=10cm]{immagini/overlap-add-mitra-1}
  \caption{Tecnica Overlapp-and-add: (a) $x[n]$ input iniziale (b) Suddivisioni di $x[n]$ in segmenti $x_0[n]$, $x_1[n]$, $x_2[n]$. Si noti che ogni sotto-sequenza è stata estesa con almeno $4\geq M-1$ zero in quanto il filtro ha lunghezza $M=5$} 
\end{figure}

\begin{figure}[H]
  \centering
      \includegraphics[width=10cm,height=7cm]{immagini/overlap-add-mitra-2}
  \caption{Convoluzioni lineari dei vari segmenti (c) $y_m[n] = h_e[n] \circledast_{N+M-1} x_me[n]$ che dovranno essere sommati al fine di ottenere $y_L[n]$ }
\end{figure}
...
\begin{equation*}
	q(x) = 11 +6x -4x^2
\end{equation*}
dove il grado dei coefficienti è ribaltato rispetto alla notazione usuale in modo che il padding possa essere operato in coda.
Definiti i vettori
\begin{equation*}
	a = [1,5,17,0,0] \qquad e \qquad b = [11,6,-4,0,0]
\end{equation*}
i coefficienti del polinomio $z(x)$ di quarto grado prodotto di $p(x)$ e di $q(x)$ si ottengono effettuando la convoluzione lineare dei vettori a e b, ovvero:
\begin{equation*}
c = a \ast b = (11,61,213,82,-68)
\end{equation*}
...
Per determinare i coefficienti di $z(x)$ si può utilizzare l'operatore DFT. Posto:
\begin{equation*}
...
\section{FFT $d$-dimensionale e immagini}
La DFT $d$-dimensionale è definita come
\begin{equation}
X_{k_1,k_2,...,k_d}=
...
\noindent Nel caso delle immagini (2-D) questa si riduce a
\begin{equation}
	F(u,v)= \sum_{x=0}^{M-1} \sum_{y=0}^{N-1} f(x,y) e^{-j2\pi(\frac{ux}{M}+\frac{vy}{N})}
\end{equation}

\noindent Abbiamo sviluppato un semplice programma in C\# che è in grado di calcolare la 2D FFT di un'immagine e qualche operazione basilare di filtraggio.\\
...
\paragraph{Fasi Elaborazione}
\begin{enumerate}
	\item Moltiplicazione dell'immagine originale $f(x,y)$ per $(-1)^{x+y}$ per centrare le basse frequenze. Proprietà \eqref{eq:frequency-shift}
	\item $F(u,v)\leftarrow DFT\; f(x,y)$
	\item editing $F(u,v)$
	\item $f(x,y)\leftarrow $IDFT\;$F(u,v)$
	\item Moltiplicazione di $f(x,y)$ per $(-1)^{x+y}$
\end{enumerate}

\paragraph{Applicazione dell'elaborazione}.\\

\noindent Qui di seguito sono riportate alcune immagini e la loro trasformata a cui è stata applicata una log-tranform per aumentare la gamma dei bianchi in modo da rendere la fft calcolata più visibile.

\begin{figure}[H]
  \centering 		             
  \subfloat[Rettangolo]{\includegraphics[width=0.6\textwidth]{immagini/rettangolo}} 
  \subfloat[Trasformata]{\includegraphics[width=0.6\textwidth]{immagini/rettangolo-fft}} 
\end{figure}

\begin{figure}[H]
  \centering \subfloat[Rettangolo Ruotato]{\includegraphics[width=0.6\textwidth]{immagini/rettangolo-ruotato}} \subfloat[Trasformata]{\includegraphics[width=0.6\textwidth]{immagini/rettangolo-ruotato-fft}}                  
\end{figure}

\begin{figure}[H]
  \centering \subfloat[Pattern Taj Mahal]{\includegraphics[width=0.6\textwidth]{immagini/render-taj-ifscompose}} \subfloat[Trasformata]{\includegraphics[width=0.5\textwidth]{immagini/render-taj-ifscompose-fft}}               
\end{figure}

\begin{figure}[H]
  \centering \subfloat[Pattern]{\includegraphics[width=0.6\textwidth]{immagini/square-noiseless}} \subfloat[Trasformata]{\includegraphics[width=0.6\textwidth]{immagini/square-noiseless-fft}}               
\end{figure}

Le seguenti immagini sono il risultato del filtraggio con il software da noi sviluppato.
Sono riportate immagine originale, fft, fft a cui sono state rimosse delle frequenze e antitrasformata.

Notiamo come la conoscenza della caratterizzazione del rumore permetta di rimuovere, ove possibile, le frequenze ad esso associato dall'immagine.

\begin{figure}[H]
  \centering \subfloat[Automobile]{\includegraphics[width=0.35\textwidth]{immagini/macchina}}  \subfloat[Trasformata]{\includegraphics[width=0.5\textwidth]{immagini/macchina-fft}} \\
  \subfloat[Automobile filtrata]{\includegraphics[width=0.35\textwidth]{immagini/macchina-filtrata}} 
\subfloat[Trasformata filtrata]{\includegraphics[width=0.5\textwidth]{immagini/macchina-filtrata-fft}}                
\end{figure}

\begin{figure}[H]
  \centering 
  \subfloat[Africa]{\includegraphics[width=0.45\textwidth]{immagini/africa}}\quad  \subfloat[Trasformata]{\includegraphics[width=0.45\textwidth]{immagini/africa-fft}} \\
  \subfloat[Africa filtrata]{\includegraphics[width=0.45\textwidth]{immagini/africa-filtrata}} \quad 
\subfloat[Trasformata filtrata]{\includegraphics[width=0.45\textwidth]{immagini/africa-filtrata-fft}}                
\end{figure}

\begin{figure}[H]
  \centering 
  \subfloat[1. Immagine dallo spazio con rumore]{\includegraphics[width=0.45\textwidth]{immagini/satellite}}\quad  \subfloat[2. Trasformata]{\includegraphics[width=0.45\textwidth]{immagini/satellite-fft}} \\
  \subfloat[4. Risultato]{\includegraphics[width=0.45\textwidth]{immagini/satellite-filtrato}} \quad 
\subfloat[3.Trasformata filtrata]{\includegraphics[width=0.45\textwidth]{immagini/satellite-filtrato-fft}}                
\end{figure}
