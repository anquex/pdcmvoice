...
\end{align}
Questo ci è utile per definire le versioni normalizzate 
\begin{equation}
	\widetilde{F}_N=\frac{1}{\sqrt{N}} F_N  \qquad \qquad  \widetilde{F}_N^*=\frac{1}{\sqrt{N}} F_N^*
\end{equation}
Una proprietà delle matrici ortogonali (e quindi che costituite da colonne ortonormali) è che \emph{conservano le norme dei vettori}.
...
\begin{equation}
\left\|\mathbf{\hat{X}-X}\right\|_2\leq 1.06 \sqrt{N} {(2N)}^{3/2} 2^{-b} \left\|\mathbf{X}\right\|_2
\end{equation}
\end{Teorema}
\begin{proof}
