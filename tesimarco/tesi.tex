...
\begin{itemize}
\item nodes \emph{id}s are initially uniformly distributed in the tree,
\item changing a part of a nodes \emph{id} is allowed if it doesn't affects the already visited conflicting sets,
\item after that a collision takes place in a node at level $\hat{N}=\lfloor\log_{2}\hat{n}\rfloor$, the nodes interested in the collision  can be redistributed in the collided domain.
\end{itemize}
...
Figure \ref{EBTvsEBTopt} shows the expected improvement in percentage between the expected cost of EBT over  the expected cost of the modified EBT. It results oscillatory with mean 1.42.\\

The \emph{EBT with optimal MBT Algorithm}, proposed in this work, is the average case best \emph{tree-based algorithm} known so far.
