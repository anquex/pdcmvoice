...
\section{GEGA BSE}
A more general form of the EGA algorithm can be obtained if we do not \emph{a-priori} limit the possible estimates. This can be simply achieved by letting $\mathcal{N}$ be the set of all the positive integer numbers.
\begin{equation*}
\mathcal{N}=\{1,2,3,...\}
\end{equation*}
...
\item Use a ``window'' of $T$ slots to refine the estimate. The idea is similar to
...
A good propriety is that the estimate is always finite.
