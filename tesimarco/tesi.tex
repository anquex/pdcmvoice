...
\section{Model}
\label{se:model}
\marginpar{\hspace{1.5cm}NOTA}
\marginpar{\hspace{1.5cm}A FIANCO}
\textcolor{red}{PARTI IN BLU AGGIUNTE O CORRETTE}\\
...
\marginpar{\hspace{1.5cm}NOTA, OK?}
...
\label{fg:comparison}
\end{figure}
We considered the average running time for both the algorithms to provide a meaningful ad fair comparison: in \emph{GEGA} $t\approx\log(n)+T$, in \emph{Cidon} $t\approx n\pc C$.   Figure \ref{fg:comparison} shows the minimum  $k$ for which $\theta_{k}$ expressed in \eqref{eq:zanella-bound} is greater than or equal to 0.99.
