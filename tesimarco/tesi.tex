...
As $i$ increases, the probability that at most one node transmits monotonically grows and approaches 1 extremely rapidly as $i$ increases past $\log_{2}n$ so we expect $i$ is close to $\log_{2}n$.\\

\begin{algorithm}[H]
\begin{algorithmic}
\STATE \COMMENT Each node runs this algorithm
\STATE $i\gets 0$
\REPEAT
	\STATE $i\gets i+1$
