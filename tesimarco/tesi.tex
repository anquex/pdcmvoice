...
\begin{center}
%resize in cm
\includegraphics[scale=1]{immagini/logo_unipd_black} %1.5 2.5
\end{center}
\end{figure}
...
\centering \expandafter{\Large{UNIVERSITÀ DEGLI STUDI DI PADOVA}\par}
\begin{center}
        \LARGE{\rm\expandafter{Facolt\`a di Ingegneria}}\par
        \large\expandafter{Corso di Laurea in Ingegneria Informatica}\par
\end{center}

%\vspace{0.5cm}
\begin{center}
        {\LARGE{Tesi di Laurea Specialistica}\par}
\end{center}
...
\begin{center}
        ...
        %\sc Stima Della Molteplicità\\ Di Insiemi Di Conflitto
        BATCH SIZE ESTIMATE
        %STIMA DELLA MOLTEPLICITÀ\\ DI INSIEMI DI CONFLITTO
        %Stima della molteplicità\\ di insiemi di conflitto
        \par}}
\end{center}
...
Relatore & & Laureando \\
Prof. \textbf{Andrea Zanella} & & \textbf{Marco Bettiol}\\
 & & {\small Matricola 586580-IF}\\
\end{tabular}
\vspace{4.5 cm}
...
\line(1, 0){338} \\
\begin{normalsize}
\textsc{Anno Accademico 2009-2010}
\end{normalsize}
\par
\vfill\par 
\clearpage
...
{\flushleft \large 
\begin{tabular}{ll}
Defended on & $28^{th}$ June, 2010\\ 
& Palazzo del Bo, Padova\\%Finished: \today,   \currenttime\\
\end{tabular}\\
\null\vspace{\stretch{2}}
\texttt{Author email: bettiol.marco at gmail dot com}
...
To adapt to any scenario, BRAs can be oblivious of the batch multiplicity $n$: in this case, however, the expected batch resolution time (often referred to as \emph{batch resolution interval, BRI}) is not optimal. In fact, the knowledge of the conflict multiplicity $n$ is the most critical factor to optimize the batch resolution and to allow the usage of advanced resolution schemes characterized by higher resolution efficiency. For this reason the \emph{Batch Size Estimate} is pivotal.\\
...
\rev{Intuitively setting $p=1/2$ can be a good choice since the algorithm is in some sense ``symmetric'':  best performance are achieved when $\mathcal{R}$ and $\mathcal{S}$ are balanced.\\}
...
\begin{figure}[H]
\begin{center}	
    \subfloat[\emph{CMBT vs MBT}: very small batch sizes]{\includegraphics[scale=0.5]{matlab/EBT/CMBT-vs-MBT-small}} 
  \subfloat[\emph{CMBT vs MBT}: larger batch sizes]{\includegraphics[scale=0.5]{matlab/EBT/CMBT-vs-MBT-large}}
  \end{center}
...
\section{EGA BSE}
The \emph{Enhanched Greenberg Algorithm (EGA)} is a first improvement over the Basic Greenberg algorithm. \\
